...
function matchPassesAdvanced(m) {
  ...
  if (f.date && localDateKey(m.date) !== f.date) return false;
  ...
  if (f.timeSlot) {
    if (f.timeSlot === "daytime" || f.timeSlot === "night") {
      if (slotSimpleKey(m.date) !== f.timeSlot) return false;
    } else if (slotKey(m.date) !== f.timeSlot) return false;
  }
  return true;
}
...
function populateFilterSelects() {
  ...
  (state.data?.matches || []).forEach((m) => {
    ...
    if (!dateMap.has(key)) {
      dateMap.set(key, { value: key, label: formatDateDay(m.date) });
    }
  });
  const dates = [...dateMap.values()].sort((a, b) => a.value.localeCompare(b.value));
  fill("#filterDate", dates, "filter.allDate");
...
function renderMatches() {
  ...
  grid.innerHTML = list
    .map((m, i) => {
      ...
      const slot = slotLabelFor(m.date);
      const range = formatRange(m.date);
      ...
      <article class="match-card ${live ? "match-card--live" : ""} ${m.recommended ? "match-card--recommended" : ""}" style="--i:${i}">
        ...
        <div class="match-card__actions">
          ${WC_DEMO ? "" : `<button type="button" class="btn-subscribe ${sub ? "btn-subscribe--active" : ""}" data-match-id="${m.id}">
...
function renderPhases() {
  ...
  el.innerHTML = cal
    .map((p) => {
      const detail = isZh()
        ? p.detail_zh || formatPhaseDetail(p.detail, true)
        : formatPhaseDetail(p.detail, false);
...
function refreshTzSelectLabels() {
  const sel = $("#tzSelect");
  if (!sel) return;
  const cur = sel.value;
  sel.innerHTML = "";
  buildTzSelectOptions().forEach(({ value, label }) => {
    const opt = document.createElement("option");
    opt.value = value;
    opt.textContent = label;
    sel.appendChild(opt);
  });
  sel.value = [...sel.options].some((o) => o.value === cur) ? cur : (isTzManual() ? String(getTzOffsetMinutes()) : WC_TZ_FOLLOW);
}

function render() {
  applyStaticI18n();
  refreshTzSelectLabels();
  if (!WC_DEMO) updateRefreshIntervalLabels();
...
async function init() {
  setupDemoMode();
  setupLang();
  if (!localStorage.getItem("wc_tz_offset")) applyLangDefaultTz();
  setupTz();
  setupTabs();
  setupFilters();
  setupFilterMore();
